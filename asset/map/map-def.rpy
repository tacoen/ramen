init -90 python:
    ...
    def map_item(obj, id, **kwargs):
        ...
        if int(cont['x']) < int(obj.prop['xmin']):
            cont['x'] = obj.prop['xmin']
        if int(cont['y']) < int(obj.prop['ymin']):
            cont['y'] = obj.prop['ymin']
        if int(cont['x']) > int(obj.prop['xmax']):
            cont['x'] = obj.prop['xmax']
        if int(cont['y']) > int(obj.prop['ymax']):
            cont['y'] = obj.prop['ymax']
    ...
    for o in obj.item.keys():
        ...
        hbox xpos int(m['x']) ypos int(m['y']):
            ...
            textbutton ico(m['ico']) style 'map_ico':
                ...
                tooltip(o, m['title'])
                action[Hide('map_screen'), Function(
                    map_goto, label=m['goto'], dist=td)]
