    ...
    def ramen_event_occuring():
        ...
        for e in h.keys():
            if ramen_event_isoccur(e):
                ...
                if goto:
                    if renpy.has_label(goto): renpy.call(goto)                

                try: goto = h[e]['jump']
                except: goto = False
                if goto:
                    if renpy.has_label(goto): renpy.jump(goto)
    ...
    class event():
        ...
        def __call__(self,what=None):
            if what is None:
                ...
                try: return rbc.event.__dict__[str(self.id)][what]
                except: return None
        ...
        def is_pass(self):
            try: return rbc.event.__dict__[str(self.id)][str('pass')]
            except: return False
            ...
            if  res:
                ...
                try: 
                    ...
                    for r in rbc.event.__dict__[id]['require'].keys():
                        ...
                        if int(mc.stat[r]) >= int(rbc.event.__dict__[id]['require'][r]):
                            res = True
                        else:
                            res = False
