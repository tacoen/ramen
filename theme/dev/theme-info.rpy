...
screen ramen_ai_menu_old():
    ...
    python:
        ...
        if tab == 'scenery' and view == 'pose': view = 'vars'
        if tab == 'npc' and view == 'pre': view = 'vars'
...
screen rai_navtab:
    ...
    python:
        topic = ['ramen', 'scenery', 'npc']

    hbox:
        for t in topic:
            textbutton t action [SetScreenVariable('tab', t), SetScreenVariable('view', 'vars')] style "rai_nav"
            null width 4
...
screen rai_guiindex(tab):
    $ topic = ['ico','view','explorer']
    hbox xalign 1.0:
        for t in topic:
            textbutton t action SetScreenVariable('view', 't') style "rai_nav"
            null width 4
        null width 32
            
    frame ysize 1 background "#cccc"
...
screen rai_viewer(obj_id, view, tab):
    ...
    vbox:
        hbox xalign 1.0:
            textbutton "predict" action SetScreenVariable('view', 'predict') style "rai_nav"
            null width 4
            textbutton "vars" action SetScreenVariable('view', 'vars') style "rai_nav"
            null width 32
    ...
    if obj_id is not None:
        ...
        vbox:
            ...
            if w_select is not None:
                ...
                python:
                    mw = 220
                    nc = math.ceil((config.screen_width - 200) / mw)
                    sp = math.ceil((config.screen_width - 200 - (nc * mw)) / nc)
                ...
                for k in obj.__dict__.keys():
                    ...
                    hbox:
                        vbox xsize 150:
                            ...
                            # text t
                        vbox:
                            text val style 'abel_font' size 16
